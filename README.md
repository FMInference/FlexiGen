# Motivation: High-Throughput Generation
This project focuses on throughput-oriented use cases of large language models (LLMs) as LLMs are getting used in new "back of house" tasks.
These tasks, such as information extraction, data cleaning, and form processing, are common in machine learning. They hold significant importance, but foundational models have not had a huge impact on them. We suspect that one reason is the lack of a supporting system.

The goal of this project is to create a high-throughput system to enable new and exciting applications of foundational models in these tasks on low-cost hardware, such as a single commodity GPU, instead of expensive systems. We demonstrate an example use case on the [HELM](https://crfm.stanford.edu/helm) benchmark.
